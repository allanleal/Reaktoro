...
using ODEFunction = std::function<int(real, VectorXrConstRef, VectorXrRef)>;

/// The function signature of the Jacobian of the right-hand side function of a system of ordinary differential equations.
using ODEJacobian = std::function<int(real, VectorXrConstRef, MatrixRef)>;
...
public:
    ...
    auto function(real t, VectorXrConstRef y, VectorXrRef f) const -> int;
    ...
    /// @return Return 0 if successful, any other number otherwise.
    auto jacobian(real t, VectorXrConstRef y, MatrixRef J) const -> int;
